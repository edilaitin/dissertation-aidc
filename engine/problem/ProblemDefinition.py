import json
import numpy
from engine.restrictions.RestrictionConflicts import RestrictionConflict, RestrictionAlphaOrBeta
from engine.restrictions.RestrictionDependences import RestrictionOneToOneDependency, \
    RestrictionOneToManyDependency, RestrictionManyToManyDependency, RestrictionManyToManyDependencyNew
from engine.restrictions.RestrictionNumberOfInstances import RestrictionUpperLowerEqualBound, \
    RestrictionRangeBound, RestrictionFullDeployment, RestrictionRequireProvideDependency
from engine.problem.Component import Component
import logging
...
class ManeuverProblem:
    ...
    def init(self, nr_vm, nr_comp):  # used at initilization for test instances
        self.nrVM = nr_vm
        self.nrComp = nr_comp
        self.D = numpy.zeros((self.nrComp, self.nrComp), dtype=numpy.int64)  # corelation hraph
        self.R = numpy.zeros((self.nrComp, self.nrComp), dtype=numpy.int64)  # conflicts graph
    ...
    def solveCPNrOfInstances(self, choosing_stategy, solutions_limit):
        ...
        from engine.solvers.impl.CP_Solver.CP_Solver_Number_of_Instances import CP_Solver_Got_Nr_Instances
    ...
    def __addOperationSystemRestriction(self):
        ...
        for comp in self.componentsList:
            if not ((self.componentsList[comp].operatingSystem is None) or (
                    len(self.componentsList[comp].operatingSystem) == 0)):
    ...
    def _addComponent(self, comp_dictionary):
        ...
        c = Component(id, comp_dictionary["name"] if "name" in comp_dictionary else None,
                      ...
                      comp_dictionary["Storage"]["StorageSize"] if "StorageSize" in comp_dictionary[
                          "Storage"] else None,
                      comp_dictionary["Storage"]["StorageType"] if "StorageType" in comp_dictionary[
                          "Storage"] else None,
                      comp_dictionary["Storage"]["StorageValue"] if "StorageValue" in comp_dictionary[
                          "Storage"] else None,
    ...
    def _getComponentsHardwareRestrictions(self):
        ...
        return [self.componentsList[compId].getComponentHardWareResources() for compId in
                range(0, len(self.componentsList))]
