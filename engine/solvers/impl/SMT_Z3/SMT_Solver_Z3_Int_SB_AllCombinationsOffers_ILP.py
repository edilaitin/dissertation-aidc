from z3 import *
from engine.solvers.impl.SMT_Z3.ilp.SMT_Solver_Z3_ILP import Z3_Solver_Int_Parent_ILP
from engine.solvers.impl.simmetryBreaking.ManuverSolver_SB import ManuverSolver_SB


class Z3_SolverInt_SB_Enc_AllCombinationsOffers_ILP(Z3_Solver_Int_Parent_ILP, ManuverSolver_SB):
    ...
    def _hardware_and_offers_restrictionns(self, scale_factor):
        ...
        tmp = []
        for k in range(self.nrVM):
            print("A machine can have only one type:",
                  sum([self.v[i * self.nrVM + k] for i in range(self.nrOffers)]) <= 1)
