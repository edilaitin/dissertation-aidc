from z3 import *
from engine.solvers.ManuverSolver import ManuverSolver
import time
...
class Z3_Solver_Int_Parent_ILP(ManuverSolver):  # ManeuverProblem):
    ...
    def RestrictionLex(self, vm_id, additional_constraints=[]):
        ...
        :param additional_constraints: other constraints
        :return:
        """
    ...
    def RestrictionConflict(self, alphaCompId, conflictCompsIdList):
        ...
        for j in range(self.nrVM):
            for conflictCompId in conflictCompsIdList:
                # self.problem.logger.debug("...{} <= 1".format([self.a[alphaCompId * self.nrVM + j],
    ...
    def run(self):
        ...
        if self.solverTypeOptimize:
            ...
            opt = Product(([self.PriceProv[o] * sum([self.v[m + o * self.nrVM] for m in range(self.nrVM)]) for o in
                            range(self.nrOffers)]))
        ...
        if status == sat:
            ...
            for vmTick in self.v:
                print("vvv", model[vmTick], "vvv")
        ...
        if self.solverTypeOptimize:
            if status == sat:
                print("priceeeee", self.PriceProv)
                print(self.solver.lower(min))
                # print("a_mat", a_mat)
