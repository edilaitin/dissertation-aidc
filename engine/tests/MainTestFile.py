...
def read_available_configurations(fileConfigurations):
    ...
    for key, value in dictionary.items():
        l = [key, value["cpu"], value["memory"], value["storage"], value["price"]]
...
def runOnce(solver, mp, outFolderDetails, repetion_number=1, sb_option=None):
    ...
    with open(outcsv, 'a', newline='') as csvfile:
        ...
        for it in range(repetion_number):
            ...
            print("min_price = {}, price_vector = {}, time = {} sec., vms_type = {}".format(min_price, price_vms, t,
                                                                                            vms_type))
    ...
    with open("./journal/" + outputFileName + ".csv", 'w', newline='') as csvfile:
        ...
        for configuration in configurations:
            ...
            for application in applications:
                ...
                for offer in offers:
                    for solverName in solvers:
                        incsv = "../journal/" + configuration + "/output_" + solverName + "/csv/" + application[
                            0] + "-" + offer + ".csv"
                        from os import listdir
                        from os.path import isfile, join
                        # onlyfiles = [f for f in listdir(incsv) if isfile(join(incsv, f))]
...
if __name__ == "__main__":
    ...
    from engine.solvers.impl.SMT_Z3.SMT_Solver_Z3_Int_SB_AllCombinationsOffers_ILP import \
        Z3_SolverInt_SB_Enc_AllCombinationsOffers_ILP
