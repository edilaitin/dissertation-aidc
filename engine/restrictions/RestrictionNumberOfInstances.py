...
class RestrictionUpperLowerEqualBound:
    ...
    def eval(self, solutionMatrix):
        ...
        if viotatedRestrictions == 1:
            ...
            for compId in self.compsIdList:
                if len(self.problem.componentsList[
                           compId].orComponentsList) and sum == 0:  # NU E CORECT TOTAL MAI GANDESTETE LA CAZUL GENERAL
                    if sum == 0:
                        s = 0
...
class RestrictionRequireProvideDependency:
    ...
    def __repr__(self):
        return "RestrictionRequireProvideDependency: each component with id {} consumes at least {} components with " \
               "id {}, " \
               "each component with id {} serves at least {} components with id {}".format(self.alphaCompId,
                                                                                           self.nAlphaBeta,
                                                                                           self.betaCompId,
                                                                                           self.betaCompId,
                                                                                           self.nBetaAlpha,
